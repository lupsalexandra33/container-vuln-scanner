...
func TestGrypeAdapter_Version(t *testing.T) {
	...
	if err := adapter.Available(ctx); err != nil {
		t.Skip("grype not available on system, skipping live version test")
	}
	...
	t.Logf("Detected Grype Version: %s, DB Built: %s", ver.Version, ver.DatabaseTimestamp)
}
...
func TestGrypeAdapter_Scan(t *testing.T) {
	...
	if err := adapter.Available(ctx); err != nil {
		t.Skip("grype not available on system, skipping live scan test")
	}
	...
	if len(res.Findings) == 0 {
		t.Errorf("expected findings from grype scan, got 0")
	}

	t.Logf("Grype scan successfully returned %d findings", len(res.Findings))
	if len(res.Findings) > 0 {
		f := res.Findings[0]
